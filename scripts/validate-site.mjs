...
assert.equal(trip.version, 'V0.3');
...
for (const [i, day] of trip.days.entries()) {
  ...
  for (const field of ['route', 'stay', 'km', 'drive', 'plan', 'road', 'charge', 'fallback']) {
...
assert.ok(trip.days[1].route.endsWith('阿尔山'));
assert.ok(trip.days[8].route.includes('恩和'));
assert.ok(trip.days[11].fallback.includes('赤峰'));
assert.ok(trip.days[12].road.includes('400–480'));
...
assert.ok(payload.trips.daxing.html.includes('V0.3'));
