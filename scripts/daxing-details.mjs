...
function inline(value) {
  const links = [];
  const text = value.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, (_, label, url) => {
...
export function extras(trip, dir) {
  ...
  const electricityMin = Math.round((minimum * .22 * 1.1 + 10) * 1.2 / 10) * 10;
  const electricityMax = Math.round((maximum * .28 * 1.15 + 20) * 2 / 10) * 10;
  const budget = [
    ['住宿', 3000, 6000, '10晚酒店×300–600；另2晚服务区条件车宿'],
    ['两人餐饮', 2080, 3380, '13天×160–260；赶路日节省可留作城镇餐费'],
    ['充电', electricityMin, electricityMax, `${minimum}–${maximum}km，22–28kWh/100km，加10–15%损耗和10–20kWh两夜暖风；1.2–2元/kWh假设`],
  ...
  const tasks = [
    ['roads', '9/23预核，9/28根河复核', '根满与伊克萨玛—白鹿岛机动车通行、桥涵、防火及车种限制', '满归镇0470-5374908；问主管管理站。失败走D4–D6备线。'],
    ['island-stay', '9/23预核，9/28最终决策', '白鹿岛接待两只成年边牧的实际房源', '白鹿岛酒店18947017554公开不可携宠；没有书面例外就排除。无其他已核房则取消穿越。'],
    ['island-charge', '9/23预核，9/28最终决策', '白鹿岛跨夜充电及D6室韦到站余量', '要桩照片、功率、使用许可与近日成功记录；抵达先试充。没有可靠方案不从根河北上。'],
    ['outbound', '9/24出发前', 'D1/D2每段主站、独立备站、同向入口与间距', '车机/运营App保存完整站名与导航公里；阿荣旗、牙克石等空档未闭环不得盲跑。'],
    ['qigan', '9/28，且D5最晚12:00判断', '奇乾许可、往返时间和额外电量', '接待方与现场主管确认；不获准或无余量直接删。'],
  ...
  for (const day of trip.days.slice(0, 12)) tasks.push([`night-${day.day}`, '9/23预订前，抵达前一晚再核', `D${day.day} ${day.date}：${day.stay}`, `${day.lodging} 请记录双犬书面接待、房型价格、取消条款、供暖；服务区记录同向侧夜停及夜厕。`]);
