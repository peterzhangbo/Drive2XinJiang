...
export function extras(trip, dir) {
  ...
  const tasks = [
    ...
    ['mordaga-stay-v07', '9/23预核，9/28最终决策', '莫尔道嘎镇接待两只成年边牧的9/29实际房源', '金秋宾馆13948095405仅平台可宠候选；确认两犬、供暖、取消条款与夜充，无可靠房源取消北穿。'],
  ...
  for (const day of trip.days.slice(0, 12)) tasks.push([`night-${day.day}${day.day <= 3 ? '-v06' : day.day <= 6 ? '-v07' : ''}`, '9/23预订前，抵达前一晚再核', `D${day.day} ${day.date}：${day.stay}`, `${day.lodging} 请记录双犬书面接待、房型价格、取消条款、供暖；服务区记录同向侧夜停及夜厕。`]);
