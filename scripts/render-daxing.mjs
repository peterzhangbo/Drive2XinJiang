...
const style = old.match(/<style>([\s\S]*?)<\/style>/)?.[1];
...
const days = trip.days.map(d => `<article class="day${[1,2,4,5,9,12,13].includes(d.day) ? ' high' : ''}" id="day-${d.day}" data-day="${d.day}"><span class="label">D${d.day} / ${d.date} · ${esc(d.type)}</span><h3>${esc(d.route)}</h3><p class="numbers">约 ${esc(d.km)} km · 纯行车 ${esc(d.drive)} h</p><small>含补能休息：${esc(d.total)}｜夜宿：${esc(d.stay)}</small><p>${esc(d.plan)}</p><details><summary>展开道路、补电与退路</summary>${['road','charge','fallback'].map(f=>`<p><b>${labels[f]}：</b>${esc(d[f])}</p>`).join('')}</details><details><summary>展开住宿、餐饮与待确认</summary>${['lodging','food','checks'].map(f=>`<p><b>${labels[f]}：</b>${esc(d[f])}</p>`).join('')}<p><a href="出行落实清单.md">查看点位来源、联系方式和确认清单</a></p></details></article>`).join('\n');
const html = `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>大兴安岭 · 满归穿越条件方案 ${trip.version}</title><style>${style}</style></head><body><main>
...
<nav class="links section-nav" aria-label="页面导航"><a href="#changes">与旧版区别</a><a href="#gates">先决条件</a><a href="#itinerary">每日行程</a><a href="#energy">电量底线</a><a href="出行落实清单.md">全部点位与联系</a><a href="基础配置要求.md">基础配置</a></nav>
